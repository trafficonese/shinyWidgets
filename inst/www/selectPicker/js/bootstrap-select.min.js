                    ...
                    if (C.clickinfo && C.multiple) {
                        if (C.clickinfo.shiftKey) {
                            if (T && T[1] && T[2].length != 0) {
                                ...
                                if ( C.$searchbox.val().length > 0 ) {
                                    // console.log("Search is changed. Must change Shift-Multi select");
                                    var selname = C.selectpicker.main.data[T[0]].text;

                                    // This was selected previously
                                    // console.log("T is " + T);
                                    // console.log("T[2][T[2].length-1] is " + T[2][T[2].length-1]);
                                    selprevshift = k.filter(function(ki) {return ki.text === T[2][T[2].length-1]})[0].posinset;
                                    //selprevshift = k.filter(function(ki) {return ki.text === T[2][0]})[0].posinset;

                                    // Get Index of just selected item
                                    selnextshift = k.filter(function(ki) { return ki.text === selname;})[0].posinset - 1
                                ...
                                var indexarr = {};
                                if (selnextshift > selprevshift) {
                                    indexarr = fillRange(selprevshift, selnextshift);
                                } else {
                                    indexarr = fillRange(selnextshift, selprevshift);
                                }
                                // console.log("indexarr is " + indexarr);
